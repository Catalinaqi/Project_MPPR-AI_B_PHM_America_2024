# src/phm_america_2024/common/logging_adapter_common.py
"""
=============================================================================
...
_LOGGER_NAMESPACE: str = "LOGGER_NAMESPACE_MPPR-AI"
...
def _build_log_file(
    ...
    if not root_path.is_absolute():
        project_root = Path(__file__).resolve().parents[3] # Sube hasta la raíz real
        logs_dir = project_root / root_path / _LOGS_DIR_NAME
    else:
        logs_dir = root_path / _LOGS_DIR_NAME

    # Paso 2: Conservamos la lógica de marcas de tiempo
    ts: str = timestamp or datetime.now().strftime(_TIMESTAMP_FMT)

    # 🛠️ Paso 3 REFACTORIZADO: Evitar doble timestamp si run_name ya termina en YYYYMMDD_HHMMSS
    # El patrón busca 8 dígitos, un guion bajo y 6 dígitos al final de la cadena
    if re.search(r"\d{8}_\d{6}$", run_name):
        filename = f"{_safe_name(run_name)}.log"
    else:
        filename = f"{_safe_name(run_name)}_{ts}.log"
