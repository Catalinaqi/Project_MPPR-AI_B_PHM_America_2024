# src/phm_america_2024/configuration/build_factory_config.py
...
class ConfigBuilder:
    ...
    def build_pipeline_config(
        ...
        pipeline_cfg = YmlRepository.load_pipeline_config(config_pipeline)
        log.debug(f"Loaded pipeline config with {len(pipeline_cfg.phases)} phases")

        # ─── ROBUST NORMALIZATION OF THE PROBLEM TYPE ─────────────────────────
        # This extracts the base type regardless of whether the file is named
        # 'regression', 'probabilistic_regression', or 'regression_pipeline_config'
        if ProblemType.REGRESSION.value in config_pipeline.lower():
            base_problem_type = ProblemType.REGRESSION.value
        elif ProblemType.CLASSIFICATION.value in config_pipeline.lower():
            base_problem_type = ProblemType.CLASSIFICATION.value
        else:
            base_problem_type = config_pipeline  # Fallback por si acaso

        pipeline_cfg.common_base_config.problem_type = base_problem_type
        log.debug(f"Normalized common_base_config.problem_type to: {base_problem_type}")
        # ────────────────────────────────────────────────────────────────────────
        ...
        return BuiltConfig(pipeline_config=pipeline_cfg, dataset_key=dataset_key)
    ...
    def _inject_runtime_vars(
        ...
        train_paths = paths.get("train") or {}
        test_path = paths.get("test") or {}
        validation_path = paths.get("validation") or {}

        train_x_path = train_paths.get("X_TRAIN_FEATURE")
        train_y_path = train_paths.get("Y_TRAIN_TARGET")
        test_x_path = test_path.get("X_TEST_FEATURE")
        validation_x_path = validation_path.get("X_VALIDATION_FEATURE")
        ...
        if "phase2_data_understanding" in pipeline_cfg.phases:
            ...
            phase2.dataset_input.x_train_path = train_x_path
            phase2.dataset_input.y_train_path = train_y_path
