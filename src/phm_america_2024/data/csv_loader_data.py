...
def _load_csv_stratified_sample(
    ...
    """
    Load a stratified sample from a large CSV by resolving strata dynamically.
    If the stratify_column is missing (e.g. reading X_train without Y_train),
    it dynamically merges the target labels from Y_train on the join_key 'id'.
    """
    resolved = resolve_path(path)
    params: Dict[str, Any] = dict(csv_params or {})
    ...
    # ── DETECCIÓN DE CONTEXTO Y COMPLEMENTARIEDAD DE Y_TRAIN ─────────────────
    # Si estamos leyendo X_train y la columna de estratificación no existe allí,
    # precargamos Y_train para poder realizar la unión en cada chunk.
    y_labels: Optional[pd.DataFrame] = None
    if "X_train" in str(resolved) and stratify_column == "faulty":
        y_path = resolve_path(str(resolved).replace("X_train.csv", "Y_train.csv"))
        if y_path.exists():
            log.info("[_load_csv_stratified_sample] Detected X_train missing '%s'. Pre-loading label registry from %s", stratify_column, y_path)
            y_labels = pd.read_csv(y_path, usecols=["id", stratify_column])
            y_labels.set_index("id", inplace=True)

    # ── FUNCIÓN INTERNA AUXILIAR PARA AYUDAR A INYECTAR LA ESTRATIFICACIÓN ───
    def _inject_strata(chunk_df: pd.DataFrame) -> pd.DataFrame:
        if stratify_column in chunk_df.columns:
            return chunk_df
        if y_labels is not None and "id" in chunk_df.columns:
            # Vincula la columna faltante usando el índice 'id' de Y_train
            return chunk_df.join(y_labels, on="id", how="inner")
        return chunk_df

    log.info("[_load_csv_stratified_sample] opening chunked reader path=%s chunksize=%d", resolved, chunksize)
    ...
    if stratify_column not in first_chunk.columns:
        log.error(
            "[_load_csv_stratified_sample] stratify_column='%s' not found. Available columns: %s",
            stratify_column, list(first_chunk.columns)
        )
        raise KeyError(
            f"stratify_column='{stratify_column}' not found in CSV columns. Available: {list(first_chunk.columns)}"
        )

    # Step 3: Compute per-stratum target counts from the first-chunk distribution estimate.
    ...
    log.info(
        "[_load_csv_stratified_sample] pre-scan found %d strata. Per-stratum targets: %s",
        n_strata, per_stratum_target
    )
    ...
    for stratum_label, group in first_chunk.groupby(stratify_column):
        ...
        collected[stratum_label].append(group.sample(n=n_take, random_state=random_state))
    ...
    for chunk in reader:
        ...
        for stratum_label, group in chunk.groupby(stratify_column):
            ...
            collected[stratum_label].append(group.sample(n=n_take, random_state=random_state + chunk_index))
            total_collected += n_take

        if total_collected >= target_total:
            log.info(
                "[_load_csv_stratified_sample] early stopping at chunk %d (collected=%d >= target=%d)",
                chunk_index, total_collected, target_total
            )
            break
    ...
    if not collected_dfs:
        log.warning("[_load_csv_stratified_sample] no rows collected — returning empty DataFrame")
        return pd.DataFrame()
    ...
    # Eliminar la columna inyectada antes de devolver df_x si originalmente no venía ahí,
    # garantizando compatibilidad con el paso de fusión subsiguiente en Step 2.1
    if y_labels is not None and stratify_column in df.columns:
        df.drop(columns=[stratify_column], errors="ignore")

    log.info(
        "[_load_csv_stratified_sample] done rows=%d cols=%d path=%s",
        len(df), df.shape[1], resolved
    )
    return df

def _load_csv_stratified_sample_(
        path: str,
        *,
        csv_params: Optional[Dict[str, Any]],
        sample_rows: int,
