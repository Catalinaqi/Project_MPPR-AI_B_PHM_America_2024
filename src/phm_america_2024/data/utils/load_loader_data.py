...
from phm_america_2024.data.read_strategy_repository_data import (
    DataSourceConfig,
    ReadStrategyContract,
)
from phm_america_2024.data.utils.csv_loader_data import load_by_strategy
...
def load_train_merged(source: DataSourceConfig) -> Tuple[pd.DataFrame, Path]:
    ...
    log.info(
        "[load_train_merged] Starting merged load for X=%s, Y=%s",
        source.x_train_path,
        source.y_train_path,
    )
    ...
    x_strategy_dict = (
        source.strategy.to_dict()
        if hasattr(source.strategy, "to_dict")
        else vars(source.strategy)
    )
    x_tmp_dict = dict(x_strategy_dict)

    if (
        x_tmp_dict.get("sample_method") == "stratified"
        and x_tmp_dict.get("stratify_column") == "faulty"
    ):
        log.info(
            "[load_train_merged] Bypassing initial 'stratified' chunking on X_train (faulty belongs to Y_train). Forcing 'random' sampling."
        )
        x_tmp_dict["sample_method"] = "random"
        x_tmp_dict["stratify_column"] = (
            ""  # Limpiamos la columna para evitar falsos positivos en csv_loader_data
        )
    ...
    log.info(
        "[load_train_merged] Optimizing Y_train read. Streaming via chunks to filter %d target IDs...",
        len(sampled_ids),
    )
    ...
    _, y_generator, _ = load_by_strategy(
        path=y_path, csv_params=source.csv_params, strategy=y_chunk_strategy
    )

    if y_generator is None:
        log.error(
            "[load_train_merged] Failed to initialize Chunked Generator for Y_train."
        )
    ...
    if not y_filtered_chunks:
        log.error(
            "[load_train_merged] No matching IDs found in Y_train for the selected X sample."
        )
