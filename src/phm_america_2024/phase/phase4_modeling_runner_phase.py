...
from phm_america_2024.model.regression_algorithm_selector_model import (
    single_probabilistic_architecture,
)
from phm_america_2024.model.classification_algorithm_selector_model import (
...
from phm_america_2024.model.classification_trainer_model import (
    ...
    post_processing_calibration,  # <-- AÑADIDO
)
...
class Phase4ModelingRunner:
    ...
    _TECHNIQUE_DISPATCH: Dict[str, Any] = {
        ...
        "post_processing_calibration": post_processing_calibration,  # <-- AÑADIDO
    ...
    def _execute_technique(
        ...
        if technique_name == "model_selection_criteria":
            ...
            # Step 4: CALL func() — execute technique, passing algorithm_config (optional)
            df_new, extra = func(
                df, tech_cfg, self.ctx, output_dir, algorithm_config=algorithm_config
            )
            log.info("[_execute_technique] '%s' completed", technique_name)
    ...
    def _persist_artifacts(
        ...
        if self.step_key == StepsPhase.STEP_4_2.value:
            # Step 1: Extract trained model and calibrator from artifacts
            trained_model = extra_artifacts.get("trained_model")
            fitted_calibrator = extra_artifacts.get("fitted_isotonic_calibrator")
            context_data[StepOutputArtifact.trained_model.value] = trained_model
            if fitted_calibrator is not None:
                context_data[StepOutputArtifact.fitted_isotonic_calibrator.value] = (
                    fitted_calibrator
                )
            log.debug(
                "[_persist_artifacts] trained_model present: %s, calibrator present: %s",
                trained_model is not None,
                fitted_calibrator is not None,
            )

        elif self.step_key == StepsPhase.STEP_4_4.value:
            # Step 2: Extract evaluation metadata from artifacts
            best_metadata = extra_artifacts.get("best_model_metadata")
            # Elegir la clave correcta según la tarea
            if task == "classification":
                artifact_key = (
                    StepOutputArtifact.best_classification_model_metadata.value
                )
            else:
                artifact_key = StepOutputArtifact.best_regression_model_metadata.value
            context_data[artifact_key] = best_metadata
            log.debug(
                "[_persist_artifacts] best_model_metadata present: %s",
                best_metadata is not None,
            )

        # Step 3: CALL update() — consolidate remaining artifacts (exclude keys already stored)
        keys_to_exclude = {
            "trained_model",
            "fitted_isotonic_calibrator",
            "best_model_metadata",
        }
        remaining = {
