...
def _save_selected_classification_train(
    ctx: Any, artifact_path: Any, **context_data: Any
) -> None:
    """Persist the selection-result DataFrame as parquet."""
    df = context_data.get(
    ...
    if hasattr(artifact_path, "get") or isinstance(artifact_path, dict):
        ...
        if not real_path_str:
            log.error(
                "[_save_selected_classification_train] Missing 'path' key in artifact configuration: %s",
                artifact_path,
            )
            raise ValueError(
                "Artifact configuration block is missing the mandatory 'path' field."
    ...
    save_parquet(df, str(full_path), compression="snappy")
    log.info(
        "[_save_selected_classification_train] Saved rows=%d to %s", len(df), full_path
    )
...
def _save_cleaned_classification_train(
    ...
    df = context_data.get(StepOutputArtifact.cleaned_classification_train_parquet.value)
    if df is None or (hasattr(df, "empty") and df.empty):
        log.warning("[_save_cleaned_classification_train] No dataframe to persist")
        return
    full_path: Path = resolve_path(ctx.phase3_dir / artifact_path)
    full_path.parent.mkdir(parents=True, exist_ok=True)
    # save_parquet(df, str(full_path))
...
def _save_engineered_regression_train(
    ...
    df = context_data.get(StepOutputArtifact.engineered_regression_train_parquet.value)
    if df is None or (hasattr(df, "empty") and df.empty):
        log.warning("[_save_engineered_regression_train] No dataframe to persist")
        return
    full_path: Path = resolve_path(ctx.phase3_dir / artifact_path)
    full_path.parent.mkdir(parents=True, exist_ok=True)
    # save_parquet(df, str(full_path))
    save_parquet(df, str(full_path), compression="snappy")
    log.info(
        "[_save_engineered_regression_train] Saved rows=%d to %s",
        len(df),
        artifact_path,
    )
...
def _save_transformed_classification_train(
    ...
    save_parquet(df, str(full_path), compression="snappy")
    log.info(
        "[_save_transformed_classification_train] Saved rows=%d to %s",
        len(df),
        artifact_path,
    )
